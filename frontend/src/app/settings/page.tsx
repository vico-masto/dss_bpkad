    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#101828] tracking-tight leading-none">Pengaturan Sistem</h1>
                         ...
                         <Table>
                            <TableHeader className="bg-[#F8F9FA]">
                               <TableRow className="border-b border-[#E9ECEF] hover:bg-transparent">
                                  <TableHead className="px-8 py-5 text-xs font-medium text-[#475467] uppercase tracking-wider">ID / Kode</TableHead>
                                  <TableHead className="px-8 py-5 text-xs font-medium text-[#475467] uppercase tracking-wider">Nama Referensi</TableHead>
                                  {activeRefTab === 'sd' && <TableHead className="px-8 py-5 text-xs font-medium text-[#475467] uppercase tracking-wider">Kategori</TableHead>}
                                  <TableHead className="px-8 py-5 text-center text-xs font-medium text-[#475467] uppercase tracking-wider">Aksi</TableHead>
                               </TableRow>
                            </TableHeader>
                            <TableBody className="divide-y divide-[#F2F4F7]">
                               {loading ? (
                                 <TableRow><TableCell colSpan={4} className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-[#98A2B3]" size={32} /></TableCell></TableRow>
                               ) : data.map((item: any) => (
                                 <TableRow key={item.id} className="hover:bg-[#F8F9FA] transition-colors group">
                                    <TableCell className="px-8 py-5 font-bold text-[#101828] text-xs uppercase">{item.id}</TableCell>
                                    <TableCell className="px-8 py-5 font-semibold text-[#475467] text-xs uppercase">{item.nama}</TableCell>
                                    {activeRefTab === 'sd' && (
                                       <TableCell className="px-8 py-5">
                                          <Badge variant="outline" className={cn("px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider", item.kategori === 'BEBAS' ? "bg-[#ECFDF3] text-[#027A48] border-[#ABEFC6]" : "bg-[#EFF8FF] text-[#175CD3] border-[#B2DDFF]")}>
                                            {item.kategori}
                                          </Badge>
                                       </TableCell>
                                    )}
                                    <TableCell className="px-8 py-5 text-center">
                                       <button onClick={() => handleDelete(item.id)} className="p-2 text-[#98A2B3] hover:text-[#F04438] transition-all"><Trash2 size={16} /></button>
                                    </TableCell>
                                 </TableRow>
                               ))}
                            </TableBody>
...
function TabBtn({ active, onClick, label, icon }: any) {
  return (
    <button onClick={onClick} className={cn("flex items-center gap-2 px-6 py-2 rounded-lg text-[11px] font-bold transition-all", active ? "bg-white text-[#101828] shadow-sm" : "text-[#667085] hover:text-[#101828]")}>
       {icon}
...
function RefTabBtn({ active, onClick, label }: any) {
  return (
    <button onClick={onClick} className={cn("px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all border uppercase tracking-wider", active ? "bg-[#101828] text-white border-[#101828]" : "bg-white text-[#98A2B3] border-[#EAECF0] hover:bg-[#F8F9FA]")}>
       {label}
    </button>
  );
}
...
function RefForm({ type, onSuccess }: any) {
  ...
  return (
    <Card className="p-8 rounded-xl border border-[#E9ECEF] shadow-sm bg-white space-y-6">
       <h4 className="text-xs font-bold text-[#101828] uppercase tracking-widest">Tambah {type === 'sd' ? 'Sumber Dana' : type === 'opd' ? 'OPD' : 'Jenis Belanja'}</h4>
       <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
             <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#667085] uppercase tracking-widest ml-1">ID / Kode</label>
                <Input type="text" className="h-11 px-4 bg-[#F9FAFB] border-[#EAECF0] rounded-lg outline-none text-xs font-bold uppercase" value={form.id} onChange={(e) => setForm({...form, id: e.target.value.toUpperCase()})} required />
             </div>
             <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#667085] uppercase tracking-widest ml-1">Nama Lengkap</label>
                <Input type="text" className="h-11 px-4 bg-[#F9FAFB] border-[#EAECF0] rounded-lg outline-none text-xs font-bold uppercase" value={form.nama} onChange={(e) => setForm({...form, nama: e.target.value.toUpperCase()})} required />
             </div>
             {type === 'sd' && (
                <div className="space-y-1.5">
          ...
          <Button type="submit" disabled={saving} className="w-full h-11 bg-[#101828] hover:bg-[#1D2939] text-white rounded-lg font-bold text-[10px] shadow-sm flex items-center justify-center gap-2 uppercase tracking-widest">
...
function UserManagementForm() {
  ...
  return (
    <Card className="p-10 rounded-xl border border-[#E9ECEF] shadow-sm bg-white space-y-8">
       <div className="flex items-center gap-4 mb-2">
          <div className="w-10 h-10 bg-[#EFF8FF] text-[#175CD3] rounded-lg flex items-center justify-center"><UserPlus size={20} /></div>
          <div>
             <h4 className="text-base font-bold text-[#101828] tracking-tight">Registrasi Akun Operator</h4>
       ...
       <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="space-y-1.5">
                ...
                <Input type="text" className="h-11 px-4 bg-[#F9FAFB] border-[#EAECF0] rounded-lg outline-none font-bold" value={form.username} onChange={(e) => setForm({...form, username: e.target.value})} required />
             </div>
             <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#667085] uppercase tracking-widest ml-1">Password</label>
                <Input type="password" placeholder="••••••••" className="h-11 px-4 bg-[#F9FAFB] border-[#EAECF0] rounded-lg outline-none font-bold" value={form.password} onChange={(e) => setForm({...form, password: e.target.value})} required />
             </div>
          </div>
          <div className="space-y-1.5">
             <label className="text-[10px] font-bold text-[#667085] uppercase tracking-widest ml-1">Level Akses (Role)</label>
             <select className="w-full h-11 px-4 bg-[#F9FAFB] border border-[#EAECF0] rounded-lg outline-none font-bold text-[#101828] text-xs appearance-none uppercase" value={form.role} onChange={(e) => setForm({...form, role: e.target.value})}>
...
function DataCard({ title, desc, icon, color }: any) {
  return (
    <Card className="p-10 rounded-xl border border-[#E9ECEF] shadow-sm bg-white flex flex-col items-center text-center space-y-6 group hover:border-[#2E90FA] transition-all cursor-pointer">
       ...
       <div>
          <h4 className="text-sm font-bold text-[#101828] mb-2 uppercase tracking-wider">{title}</h4>
          <p className="text-xs font-medium text-[#667085] max-w-[200px] leading-relaxed">{desc}</p>
       </div>
       <Button className={cn("w-full h-11 rounded-lg font-bold text-[10px] transition-all uppercase tracking-widest", color === 'indigo' ? "bg-[#101828] hover:bg-[#1D2939] text-white" : "bg-[#027A48] hover:bg-[#05603A] text-white")}>
          Eksekusi Sekarang
       </Button>
    </Card>
  );
