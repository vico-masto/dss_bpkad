    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
          ...
          {currentUser?.role === 'admin' && (
            <Dialog open={isAddingUser} onOpenChange={setIsAddingUser}>
              <DialogTrigger asChild>
                <Button className="bg-[#101828] hover:bg-[#1D2939] text-white rounded-xl font-bold flex items-center gap-2 px-6 h-11 shadow-xl shadow-sm">
                  <UserPlus size={18} />
              ...
              <DialogContent className="sm:max-w-[425px] rounded-[32px] border-none shadow-2xl">
                ...
                <div className="grid gap-6 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={newUser.role} onValueChange={(v) => setNewUser({...newUser, role: v})}>
                      <SelectTrigger className="h-11 rounded-xl bg-fin-page border-fin-border font-bold">
                      ...
                      <SelectContent className="rounded-xl border-fin-border shadow-xl">
                        ...
                        <SelectItem value="Operator SP2D">Operator Pengeluaran (SP2D)</SelectItem>
                ...
                <DialogFooter>
                  <Button onClick={handleAddUser} className="w-full bg-[#101828] hover:bg-[#1D2939] text-white font-black text-xs uppercase tracking-widest h-12 rounded-xl shadow-lg">
                    Daftarkan Akun
                  </Button>
                </DialogFooter>
              </DialogContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="p-8 border-fin-border shadow-sm space-y-6 rounded-[32px] bg-fin-surface">
            ...
            <div className="space-y-5">
               ...
               <Button onClick={handleChangePassword} className="w-full bg-[#101828] hover:bg-[#1D2939] text-white h-12 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-xl shadow-sm">
                 Simpan Perubahan
               </Button>
            </div>
          </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-fin-border shadow-sm rounded-[32px] overflow-hidden bg-fin-surface">
            <div className="p-8 bg-fin-surface border-b border-fin-border flex items-center justify-between">
               <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-fin-page text-fin-text-muted flex items-center justify-center border border-fin-border">
                    <ShieldCheck size={22} />
                  </div>
                  <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-fin-page text-[10px] font-black text-fin-text-muted uppercase tracking-[0.2em]">
                    <th className="px-8 py-5">User & Authority</th>
                ...
                <tbody className="divide-y divide-[#F2F4F7]">
                  {isLoading ? (
                    ...
                    <tr key={u.id} className="hover:bg-fin-page/50 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                           <div className={cn(
                             "w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xs shadow-sm",
                             u.role === 'admin' ? "bg-indigo-600 text-white" : "bg-[#ECFDF3] text-[#027A48]"
                           )}>
                           ...
                           <div>
                              <p className="text-sm font-black text-fin-text-primary flex items-center gap-2 tracking-tight">
                                {u.username}
                                {u.id === currentUser?.id && (
                                  <Badge className="bg-[#F2F4F7] text-fin-text-muted hover:bg-[#F2F4F7] text-[8px] font-black px-1.5 py-0 rounded">CURRENT</Badge>
                                )}
                              </p>
                         ...
                         {currentUser?.role === 'admin' ? (
                           <div className="flex items-center justify-center gap-2">
                             <Button 
                               ...
                               variant="ghost" 
                               size="icon" 
                               className="h-10 w-10 rounded-xl text-fin-text-muted hover:text-indigo-600 hover:bg-indigo-50 border border-transparent hover:border-indigo-100 transition-all"
          ...
          <Card className="p-6 bg-fin-page border-fin-border flex items-start gap-4 rounded-[32px] border-dashed">
      ...
      <Dialog open={isResetDialogOpen} onOpenChange={setIsResetDialogOpen}>
        <DialogContent className="sm:max-w-[400px] rounded-[32px] border-none shadow-2xl">
          ...
          <DialogFooter>
            <Button onClick={handleResetPassword} className="w-full bg-[#101828] hover:bg-[#1D2939] text-white font-black text-xs uppercase tracking-widest h-12 rounded-xl shadow-lg">
              Konfirmasi Reset
            </Button>
          </DialogFooter>
        </DialogContent>
