...
async function checkFinal() {
  const dates = ['2026-03-31', '2026-04-30'];
  ...
  for (const d of dates) {
    ...
    const [sa, inc, exp, taxPot, adjIn, adjOut] = await Promise.all([
      ...
      prisma.$queryRaw`
        SELECT SUM(CASE WHEN h.status_rekon = 'SUDAH_BRUTO' THEN d.nilai_bruto ELSE (d.nilai_bruto - (COALESCE((SELECT SUM(nilai) FROM data_sp2d_potongan p WHERE p.id_sp2d = h.id), 0) * (d.nilai_bruto / NULLIF(h.nilai_bruto, 0)))) END) as total 
