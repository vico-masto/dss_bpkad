...
async function checkCrossMonthInv() {
  ...
  const crossPot = await prisma.$queryRaw`
    SELECT p.nomor_sp2d, p.nilai, b.tanggal as bank_tgl, p.tanggal_pencairan as bku_tgl
  ...
  console.log('TOTAL CROSS MONTH INV (Future BKU in Current Bank):', sum);
}

checkCrossMonthInv().finally(() => prisma.$disconnect());
