...
export default function DiscrepancyReportPage() {
  ...
  const Section = ({ id, title, icon: Icon, color, children }: any) => {
    ...
    return (
      <Card className="rounded-2xl border border-[#E9ECEF] shadow-sm overflow-hidden">
        <button onClick={() => setActiveSection(isOpen ? null : id)} className="w-full flex items-center justify-between px-6 py-4 bg-white hover:bg-[#F8F9FA] transition-colors">
          ...
          {isOpen ? <ChevronDown size={16} className="text-[#98A2B3]" /> : <ChevronRight size={16} className="text-[#98A2B3]" />}
        </button>
        <AnimatePresence>
          {isOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
              <div className="border-t border-[#F1F3F5]">{children}</div>
            </motion.div>
          )}
        </AnimatePresence>
      </Card>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col gap-5">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#101828]">Discrepancy Audit Center</h1>
          <p className="text-sm text-[#475467] mt-1">Investigasi selisih, anomali bank, dan rekonsiliasi yang belum tuntas</p>
        </div>

        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:justify-end">
            <Select value={year} onValueChange={setYear}>
              <SelectTrigger className="w-32 h-10 text-sm font-semibold border-[#E9ECEF]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Button variant="outline" size="sm" onClick={() => mutate()} className="h-10 gap-2">
              <RefreshCw size={14} className={isLoading ? 'animate-spin' : ''} />
              Refresh
            </Button>
            <Button onClick={handleExport} disabled={!data} className="h-10 bg-white text-slate-700 border-[#E9ECEF] hover:bg-slate-50 gap-2">
              <Download size={14} />
              Excel
            </Button>
            <Button onClick={() => setShowBarModal(true)} disabled={!data} className="h-10 bg-indigo-600 text-white gap-2 hover:bg-indigo-700 shadow-md shadow-indigo-100">
              <FileText size={14} />
              Generate BAR
            </Button>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {[
          { label: 'SP2D Belum Rekon', value: formatCurrency(totalSP2DBelum), sub: 'BKU (Keluar)', color: 'border-b-rose-500', icon: XCircle, iconColor: 'text-rose-500' },
          { label: 'Bank Debet Blm Cocok', value: formatCurrency(totalBankDebetBelum), sub: 'Bank (Keluar)', color: 'border-b-amber-500', icon: AlertTriangle, iconColor: 'text-amber-500' },
          { label: 'Penerimaan Blm Rekon', value: formatCurrency(totalPenerimaanBelum), sub: 'BKU (Masuk)', color: 'border-b-emerald-500', icon: CheckCircle2, iconColor: 'text-emerald-500' },
          { label: 'Bank Kredit Blm Cocok', value: formatCurrency(totalBankKreditBelum), sub: 'Bank (Masuk)', color: 'border-b-blue-500', icon: BarChart3, iconColor: 'text-blue-500' },
          { label: 'Potongan Blm Match', value: formatCurrency(totalPotonganBelum), sub: 'Rincian Pajak', color: 'border-b-violet-500', icon: ShieldCheck, iconColor: 'text-violet-500' },
          { label: 'OPD Selisih', value: `${opdDenganSelisih} OPD`, sub: 'Audit Perlu', color: 'border-b-slate-500', icon: Building2, iconColor: 'text-slate-500' },
        ].map((c, i) => (
          <Card key={i} className={cn("p-5 rounded-2xl border border-[#E9ECEF] shadow-sm border-b-2", c.color)}>
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[9px] font-bold text-[#98A2B3] uppercase tracking-widest">{c.label}</p>
                <p className="text-lg font-black text-[#101828] mt-1">{(isLoading && !data) ? '...' : c.value}</p>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#F8F9FA]">
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Bulan</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Penerimaan BKU</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Pengeluaran BKU</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Saldo Akhir Bank</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Selisih</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody className="divide-y divide-[#F1F3F5]">
                  {(data.monthlyBalance || []).map((r: any) => {
                    ...
                    return (
                      <TableRow key={r.bulan} className={cn("transition-colors", hasIssue ? "bg-[#FFFAEB]/50" : "")}>
                        <TableCell className="text-xs font-bold text-[#101828] py-3">{MONTHS[r.bulan - 1]} {year}</TableCell>
                        <TableCell className="text-xs text-right text-[#12B76A] font-semibold py-3">{formatCurrency(toN(r.penerimaan))}</TableCell>
                        <TableCell className="text-xs text-right text-[#101828] font-semibold py-3">{formatCurrency(toN(r.pengeluaran))}</TableCell>
                        <TableCell className="text-xs text-right text-[#2E90FA] font-semibold py-3">{formatCurrency(toN(r.saldo_bank))}</TableCell>
                        <TableCell className={cn("text-xs text-right font-bold py-3", hasIssue ? "text-[#F04438]" : "text-[#12B76A]")}>
          ...
          <Section id="penerimaan" title="Rincian Penerimaan (STS) Belum Sinkron ke Bank" icon={CheckCircle2} color="bg-emerald-600">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#F8F9FA]">
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Tanggal</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Referensi STS</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">OPD</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Uraian Pendapatan</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Nilai STS</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody className="divide-y divide-[#F1F3F5]">
                  {(data.penerimaanUnmatched || []).length === 0 ? (
                    ...
                    <TableRow key={i} className="hover:bg-emerald-50/30 transition-colors">
                      <TableCell className="text-xs py-3">{r.tanggal ? format(new Date(r.tanggal), 'dd/MM/yy') : '-'}</TableCell>
                      <TableCell className="text-xs font-bold py-3 text-emerald-700">{r.bukti}</TableCell>
                      <TableCell className="text-xs py-3">{r.opd}</TableCell>
                      <TableCell className="text-xs py-3 max-w-[250px] truncate">{r.uraian}</TableCell>
                      <TableCell className="text-xs text-right font-black text-emerald-600 py-3">{formatCurrency(toN(r.total_nilai))}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <Section id="bank-kredit" title="Mutasi Bank Masuk (Kredit) Tanpa Pasangan BKU" icon={Sparkles} color="bg-blue-600">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#F8F9FA]">
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Tanggal Bank</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Deskripsi Rekening Koran</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Nilai Masuk</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-center">Analisis AI</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody className="divide-y divide-[#F1F3F5]">
                  {(data.bankKreditUnmatched || []).length === 0 ? (
                    ...
                    <TableRow key={i} className="hover:bg-blue-50/30 transition-colors">
                      <TableCell className="text-xs py-3">{r.tanggal ? format(new Date(r.tanggal), 'dd/MM/yy') : '-'}</TableCell>
                      <TableCell className="text-xs font-bold py-3">{r.uraian || r.keterangan}</TableCell>
                      <TableCell className="text-xs text-right font-black text-blue-600 py-3">{formatCurrency(toN(r.total_kredit))}</TableCell>
                      <TableCell className="py-3 text-center">
                         <Badge className="bg-blue-100 text-blue-700 border-none text-[8px] font-black">PENDAPATAN YATIM</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#F8F9FA]">
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Tanggal</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Bukti</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467]">Keterangan</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-right">Selisih</TableHead>
                    <TableHead className="text-[10px] font-bold uppercase text-[#475467] text-center">Aksi</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody className="divide-y divide-[#F1F3F5]">
                  {(data.matchedWithDiscrepancy || []).map((r: any, i: number) => (
                    <TableRow key={i} className="hover:bg-indigo-50/30 transition-colors">
                      <TableCell className="text-xs font-medium text-[#475467] py-3">{r.tanggal ? format(new Date(r.tanggal), 'dd/MM/yyyy') : '-'}</TableCell>
                      <TableCell className="text-xs font-bold text-[#101828] py-3">{r.bukti || '-'}</TableCell>
                      <TableCell className="text-xs text-[#475467] py-3">{r.keterangan_rekon || r.uraian}</TableCell>
                      <TableCell className="text-xs text-right font-black text-rose-600 py-3">{formatCurrency(toN(r.selisih))}</TableCell>
                      <TableCell className="text-center py-3">
                         <Button variant="ghost" size="sm" className="h-7 px-2 text-[9px] font-bold text-indigo-600 hover:bg-indigo-50" onClick={() => { setSelectedAnomaly(r); setAuditNote(r.keterangan_rekon || ''); setAuditStatus(r.status_rekon || 'OPEN'); }}>
              ...
              {selectedAnomaly && (
                 <div className="space-y-4">
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase">Nilai Selisih</p>
                       <p className="text-lg font-black text-rose-600">{formatCurrency(toN(selectedAnomaly.selisih))}</p>
                       <p className="text-[10px] text-slate-500 mt-1">{selectedAnomaly.uraian}</p>
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase">Penjelasan</label>
                       <Textarea value={auditNote} onChange={(e) => setAuditNote(e.target.value)} placeholder="Masukkan penjelasan audit..." className="min-h-[100px] text-xs font-bold rounded-xl" />
                    </div>
                 </div>
              )}
           </div>
           <DialogFooter className="p-6 bg-slate-50 border-t flex gap-3">
              <Button variant="ghost" onClick={() => setSelectedAnomaly(null)} className="flex-1 font-black text-xs uppercase">Batal</Button>
              <Button className="flex-1 bg-slate-900 text-white font-black text-xs uppercase" onClick={async () => {
                 if (!selectedAnomaly) return;
                 setIsSubmitting(true);
                 try {
                    await api.post('/reports/reconciliation/save-resolution', { type: selectedAnomaly.tipe, id: selectedAnomaly.id, note: auditNote, status: 'RESOLVED' });
                    toast.success('Resolusi Berhasil Disimpan');
                    mutate();
                    setSelectedAnomaly(null);
                 } catch (err) { toast.error('Gagal menyimpan'); } finally { setIsSubmitting(false); }
